...
fn check_seq_matches(c: &Claim, seq: u64, path: &Path, repair: bool) -> Result<()> {
    if c.seq == seq || repair {
        return Ok(());
    }
    Err(anyhow!(
...
fn check_content_hash(c: &Claim, seq: u64, path: &Path, repair: bool) -> Result<()> {
...
fn check_integrity_mac(c: &Claim, seq: u64, path: &Path, root: &Path, repair: bool) -> Result<()> {
